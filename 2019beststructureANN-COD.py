# !/usr/bin/python3
# @File: 2019beststructureANN-COD.py
# --coding:utf-8--
# @Author:Blazer
# @Time: 2020年08月04日06时15分46秒
# 说明: 应用遗传算法优化后的最优网络结果模拟
...
def build_model():
    model = keras.Sequential([
        layers.Dense(30, activation=tf.nn.sigmoid, input_shape=[len(train_data.keys())]),
...
# 添加标题和坐标轴标签
plt.title('PCA-GA-ANN hybrid model predict COD')
...
import datetime
plt.savefig('E:/pycharm project/小论文/结果图表/' + datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S") + 'COD-scheme3.jpg')
plt.show()
